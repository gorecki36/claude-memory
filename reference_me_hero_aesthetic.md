...
**2026-06-29 update — the family is sophisticated editorial; NEVER cartoon/mascot.**
Vas showed the live hero collection. The whole set shares one sensibility: tasteful, muted, gallery/editorial. It spans MORE than the two aesthetics above: (a) painterly/photographic SUBJECTS (the charcoal lion, stacks of cash), (b) matte 3D ceramic OBJECTS (the lab flasks), and (c) loose black-ink CONCEPT sketches on warm grey with one muted accent (the "$11 a month" descending-line chart, the "lockdown" torn-document). HARD RULE: a hero is never a cartoon/mascot character scene. Cute robots, big-eyed figures, toddler/character illustrations, and "a person doing a task" cartoon vignettes all read as foreign and get rejected. Caught on the botsitting issue (2026-06-29) after generating a cutesy ink robot, a toddler-robot, and a cartoon "mentor + robot" scene, all off-brand.
Also correct the layer rule above: LEAD pieces are NOT always painterly. Recent leads ("$11", "lockdown") used the black-ink concept-sketch style (Aesthetic 2) on a warm grey ground with a muted brick-red accent. So for a lead, choose painterly-subject OR ink-concept-sketch; just keep it in the editorial family and never cartoon. Working example: `2026-06-29-botsitting-highchair.json` (plain boxy robot in a baby's high chair, Sharpie on grey, one brick-red accent). Vary palette/background from the immediately prior issue (the charcoal lion ran the week before, so botsitting went light grey + brick red).
